...
New-Item -ItemType Directory -Force -Path $VIDEO_DIR | Out-Null
New-Item -ItemType Directory -Force -Path $AUDIO_DIR | Out-Null
...
foreach ($step in $pipelineSteps) {
    $scriptPath = Join-Path $NOTEBOOK_DIR $step

    Write-Host "Ejecutando: $step"
